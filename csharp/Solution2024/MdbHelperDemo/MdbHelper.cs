...
namespace MdbHelperDemo
{
    public class MdbInstance
    {
        ...
        static MdbInstance()
        {
            ...
            //设置关系数据库对象前缀
            _rdbstablepre = "ConsoleApp2";
            //设置关系数据库连接字符串
            _connectionstring = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + AppDomain.CurrentDomain.BaseDirectory + @"\BarCode.mdb;";
        }
        ...
        public string RunSql(string sql)
        {
            ...
            {
                OleDbConnection conn = new OleDbConnection(MdbInstance.ConnectionString);
                conn.Open();
                using (OleDbTransaction trans = conn.BeginTransaction())
                {
                    ...
                    foreach (string item in sqlList)
                    {
                        if (!IsNullOrWhiteSpace(item))
                        {
                            try
                            {
                                MdbInstance.ExecuteNonQuery(CommandType.Text, item);
                                trans.Commit();
                            }
                            catch (Exception ex)
                            {
